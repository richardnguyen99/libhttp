...
static int IsStringInBiggerString(const char *longstring, const char *shortstring)
{
	...
	while (*lptr)
	{
		...
		while (*lptr && *sptr && (*lptr == *sptr))
		{
			...
			switch (*sptr)
			{
			case '*': /* we encountered a wild-card */ return UnityStrictMatch ? 0 : 1;

			case ',': /* we encountered the end of match string */
			case '"':
			case '\'':
			case 0: return (!UnityStrictMatch || (*lptr == 0)) ? 1 : 0;

			case ':': /* we encountered the end of a partial match */ return 2;
